...
func uniqueValuePipeline(fields []string, match, unwindMatch bson.D, skip, limit int64, schemaInfo types.SchemaInfo) mongoDB.Pipeline {
	...
	for _, field := range fields {
		...
		if isArray && !handledArrays[arrayPath] {
			...
			pipeline = append(pipeline,
				...
				bson.D{{Key: "$match", Value: unwindMatch}},
			)
		}
	}
