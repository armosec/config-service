...
type routerOptions[T types.DocContent] struct {
	...
	servePostV2ListRequests   bool                      //default false, when true  /<path>/v2listrequest with V2ListRequest is served
...
func AddRoutes[T types.DocContent](g *gin.Engine, options ...RouterOption[T]) *gin.RouterGroup {
	...
	if opts.servePostV2ListRequests {
		routerGroup.POST("/v2listrequest", HandlePostV2ListRequest[T])
	}
	//add array handlers
...
func (b *RouterOptionsBuilder[T]) WithPostV2ListQuery(servePostV2ListRequests bool) *RouterOptionsBuilder[T] {
	b.options = append(b.options, func(opts *routerOptions[T]) {
